...
#define DEVICE_NAME                     "freedrum"                             /**< Name of device. Will be included in the advertising data. */
...
#define PNP_ID_VENDOR_ID                0x1917                                      /**< Vendor ID. */
#define PNP_ID_PRODUCT_ID               0xEEE1                                      /**< Product ID. */
...
static void battery_level_update(void)
{
    ...
    battery_level = 1;//(uint8_t)sensorsim_measure(&m_battery_sim_state, &m_battery_sim_cfg);
 ...
 */
static void led_write_handler(ble_midi_t * p_lbs, uint8_t led_state)
{
    if (led_state)
    {
...
static ble_midi_t                        m_lbs;                                      /**< LED Button Service instance. */
...
static void services_init(void)
{
    ...
    err_code = ble_midi_init(&m_lbs, &init);
    APP_ERROR_CHECK(err_code);
    //hids_init();
}
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    ble_midi_on_ble_evt(&m_lbs, p_ble_evt);
}
...
static ble_uuid_t m_adv_uuids[] = {{LBS_UUID_SERVICE, 0}}; /**< Universally unique service identifiers. */
...
static void advertising_init(void)
{
    ...
    // Add service.
    ble_uuid128_t base_uuid = {APPLE_MIDI_UUID_BASE};
    err_code = sd_ble_uuid_vs_add(&base_uuid, &m_lbs.uuid_type);
    APP_ERROR_CHECK(err_code);
    m_adv_uuids[0].type=m_lbs.uuid_type;
...
int mainstuff_send_midi_note(int note, int velocity) {

	if (m_conn_handle != BLE_CONN_HANDLE_INVALID) {
		ble_midi_send_note(&m_lbs, note, velocity);
...
static void bsp_event_handler(bsp_event_t event)
{
    ...
    switch (event)
    {
        ...
        case BSP_EVENT_KEY_0:
            if (m_conn_handle != BLE_CONN_HANDLE_INVALID)
            {
            	ble_midi_send_note(&m_lbs,38,80);
...
int main(void)
{
    ...
    // Initialize.
    app_trace_init();
    timers_init();//battery measurement
    erase_bonds=false;
